...
<html lang="en">
 ...
 <!--           <link rel="shortcut icon" href="favicon.ico" />-->
		...
		{
			$q=mysql_query("select * from photo where album_id=".$_REQUEST['delid']);
			while($r=mysql_fetch_array($q))
			{
				unlink("album_img/m/".$r['photo_name']);
				unlink("album_img/".$r['photo_name']);
			}
			mysql_query("delete from photo where album_id=".$_REQUEST['delid']);
			mysql_query("delete from album_add where album_id=".$_REQUEST['delid']);
		}
		if(isset($_POST['submit']))
		{
			if(isset($_REQUEST['eid']))
			{
				mysql_query("update album_add set album_name='".$_POST['album']."' where album_id=".$_REQUEST['eid']);
			}
			else
			{ 					
				mysql_query("insert into album_add values(NULL,'".$_POST['album']."')");
			}
		}
		if(isset($_REQUEST['edid']))
		{
			$eid=$_REQUEST['edid'];
			$q=mysql_query("select * from album_add where album_id=".$eid);
	...
	<?php include('header.php');?>
            ...
            <div id="contentwrapper">
										...
										<fieldset>
											...
											<div class="control-group">
												...
												<div class="controls">
													...
													?>
											<input type="text" name="album" value="<?php echo $aname; ?>"/>
												</div>
											</div>
				...
				<div class="row-fluid">
						<div class="span12">
							<h3 class="heading">Albums View</h3>
							<table class="table table-bordered table-striped table_vam" id="dt_gal">
								...
								<tbody>
								<?php
				$qr=mysql_query("select * from album_add");
				while($row=mysql_fetch_array($qr))
				{
					$r1=mysql_fetch_array(mysql_query("select count(*) from photo where album_id=".$row['album_id']));
				?>
									<tr>
										<td><?php echo $row[0];?></td>
										<td><?php echo $row[1];?></td>
										<td>( <?php echo $r1[0];?> ) <a href="photo_add.php?aid=<?php echo $row[0]; ?>">Add Photos</a></td>
										<td>
